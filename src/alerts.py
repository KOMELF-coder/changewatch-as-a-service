...
from .reliability import alert_decision
...
def validate_alert_options(data: dict) -> dict:
    ...
    confirmation = data.get("confirmation_runs")
    if confirmation is not None and (type(confirmation) is not int or not 1 <= confirmation <= 10):
        raise ValueError("confirmation_runs must be an integer from 1 to 10")
    cooldown = data.get("alert_cooldown_hours", 24)
    if type(cooldown) is not int or not 1 <= cooldown <= 720:
        raise ValueError("alert_cooldown_hours must be an integer from 1 to 720")
...
def render_alert(
    result: dict, language: str, *, html: bool = False, timezone: str = "Europe/Paris"
) -> tuple[str, str]:
    french = language == "fr"
    name = " ".join(result["competitor"].split())
    ...
    if html:
        return subject, render_html(
            result, language, cards[0]["title"], [], summary, action, cards=cards, timezone=timezone
        )
    lines = [
        "ChangeWatch",
    ...
    lines.extend(
        [
            ...
            result["url"],
            "",
            display_time(result["detected_at"], french, timezone),
...
async def apply_alert(result: dict, options: dict | None = None, sender=None) -> None:
    ...
    reason = alert_decision(result, options)
    triggered = reason in {"eligible", "no_client_email"}
    result["alert_decision_reason"] = reason
    result.update(
    ...
    log.info(
        "Alert decision: changed=%s score=%s threshold=%s generated=%s reason=%s",
        result["changed"],
    ...
    if not triggered:
        log.info("Email not sent: %s.", reason)
        return
    result["alert_subject"], result["alert_body"] = render_alert(
        result, options["language"], timezone=options["timezone"]
    )
    _, result["alert_html"] = render_alert(
        result, options["language"], html=True, timezone=options["timezone"]
    )
