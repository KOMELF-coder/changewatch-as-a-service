...
Semantic product-link fallback supplements known card selectors. It looks for a same-origin detail URL (`/p/`, `/product/`, `/products/`, `/produit/`, `/produits/`), visible product title and monetary evidence in a bounded single-product container. Image and title links can resolve to the same canonical URL; fragments are ignored. Optional `data-product-number` joins the existing SKU/ID attributes. Identity remains SKU/ID → product URL → title → stable context, never price or position. Ambiguous multiple prices preserve the entity with null price/currency. Navigation, filter/cart/checkout/help containers and comparison controls are excluded; multiple-product wrappers are not products. Hidden visual price duplicates and button/label noise are removed before extraction. Reviews are not required.

The IKEA-like regression fixtures model nested div cards with BAGGEBO / PÄRKLA titles, French comma prices, accessible price text and `?page=2#products-page-2` links. A synthetic 351-product collection spans 15 pages and tests later-page changes and reordering. This is not an official IKEA compatibility guarantee. In the read-only live check for this change, first-page extraction returned 24 products, but IKEA's explicit `?page=2` HTML repeated that batch: the scan correctly reported 2 fetched pages, 24 unique entities and `partial` / repeated-page diagnostics. All 351 products cannot be claimed from that response. Apify may receive different HTML; verify its actual results. No private API or browser fallback is attempted.

For this release, rebuild the existing Apify Actor from `main` and select the successful build in the saved test Task. Preserve the named KVS and secrets. Use an isolated client ID and `https://www.ikea.com/fr/fr/cat/lowest-price/`; omit `client_email` for an extraction-only test. Acceptance: approximately the current first-batch count is extracted (24 in the observed HTML), BAGGEBO/PÄRKLA have names, stable URLs and correct EUR prices, and the page-2 link is followed once without its fragment. If the server returns distinct batches through the last page, expect around 15 pages / 351 entities and `complete` for that catalog size. If it repeats, fails or needs JS, require honest partial/unsupported diagnostics and preserved removal safeguards instead. An unchanged rerun must not generate false product changes. Catalog sizes/prices can change; compare to the current source. Use the synthetic fixture tests to verify full 351-product aggregation independently of the live server.
